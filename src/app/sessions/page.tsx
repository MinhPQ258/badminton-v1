      ...
      <div className="space-y-8">
        <section>
          <h2 className="text-xl font-semibold text-foreground mb-4 flex items-center gap-2">
            <CalendarDays className="h-5 w-5 text-primary" /> Sắp diễn ra
          </h2>
          ...
          {upcomingSessions && upcomingSessions.length > 0 ? (
            ...
            <p className="text-muted-foreground italic">Không có buổi đánh nào sắp tới.</p>
          )}
        </section>

        <section>
          <h2 className="text-xl font-semibold text-foreground mb-4 flex items-center gap-2">
            <CalendarDays className="h-5 w-5 text-muted-foreground" /> Đã qua
          </h2>
          ...
          {pastSessions && pastSessions.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {pastSessions.map((session) => (
                <Link key={session.id} href={`/sessions/${session.id}`} className="block h-full">
                  <Card className="hover:bg-secondary transition-colors h-full opacity-80">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base line-clamp-1">{formatDate(session.start_time)}</CardTitle>
                      <CardDescription>
                        {session.status === 'settled' ? (
                          <span className="inline-block px-2 py-0.5 bg-secondary text-foreground text-xs font-medium rounded-full mt-1">Đã quyết toán</span>
                        ) : (
                          <span className="inline-block px-2 py-0.5 bg-primary/20 text-primary text-xs font-medium rounded-full mt-1">Hoàn thành</span>
                        )}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="text-sm text-muted-foreground space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="flex items-center gap-1"><Users className="w-4 h-4 text-muted-foreground" /> {session.total_attendees || 0}</span>
