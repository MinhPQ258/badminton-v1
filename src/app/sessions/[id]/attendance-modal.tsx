  ...
  return (
    <>
      <button 
        ...
        className="w-full mt-3 p-3 bg-primary/10/50 hover:bg-primary/10 border border-blue-100 rounded-lg flex items-center justify-between transition-colors group text-left"
      >
        <div className="flex items-center gap-3">
          <div className="flex -space-x-2">
            <div className="h-8 w-8 rounded-full bg-blue-100 border-2 border-white flex items-center justify-center text-primary font-semibold text-xs z-20">
              <Users className="h-4 w-4" />
            </div>
            <div className="h-8 w-8 rounded-full bg-primary/20 border-2 border-white flex items-center justify-center text-primary font-semibold text-xs z-10">
              <CheckCircle2 className="h-4 w-4" />
            </div>
          </div>
          <div>
            <p className="text-sm font-medium text-foreground">Chi tiết thành viên</p>
            <p className="text-xs text-muted-foreground">{totalRsvp} Đăng ký • {totalAttended} Đã điểm danh</p>
          </div>
        </div>
        <span className="text-xs font-medium text-primary group-hover:underline">Xem danh sách</span>
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center p-4 border-b border-border shrink-0">
              ...
              <button onClick={() => setIsOpen(false)} className="text-muted-foreground hover:text-muted-foreground bg-secondary hover:bg-secondary p-1 rounded-full transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
